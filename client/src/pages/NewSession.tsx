    ...
    <div ref={ref} style={{ position: "relative" }}>
      <button
        ...
        style={{
          ...
          borderRadius: "8px", background: open ? "oklch(68% 0.12 75 / 0.15)" : "oklch(14% 0 0)",
          border: `1px solid ${open ? "oklch(68% 0.12 75 / 0.5)" : "oklch(24% 0 0)"}`,
          color: "oklch(68% 0.12 75)", fontSize: "12px", fontWeight: 600,
      ...
      {open && (
        <div style={{
          ...
          minWidth: "300px", maxWidth: "420px", background: "oklch(12% 0 0)",
          border: "1px solid oklch(22% 0 0)", borderRadius: "12px",
          boxShadow: "0 12px 40px oklch(0% 0 0 / 0.6)", zIndex: 100, overflow: "hidden",
        }}>
          <div style={{ padding: "10px 12px", borderBottom: "1px solid oklch(18% 0 0)", display: "flex", alignItems: "center", gap: "8px" }}>
            ...
            <input
              ...
              style={{ flex: 1, background: "transparent", border: "none", outline: "none", fontSize: "13px", color: "oklch(88% 0 0)", fontFamily: "var(--font-sans)" }}
            />
            {searchTerm && (
          ...
          <div style={{ maxHeight: "260px", overflowY: "auto" }}>
            {isLoading && <div style={{ padding: "16px", textAlign: "center", fontSize: "13px", color: "oklch(45% 0 0)", fontFamily: "var(--font-sans)" }}>Loading meetings...</div>}
            ...
            {!isLoading && !queryError && (!meetings || meetings.length === 0) && <div style={{ padding: "16px", textAlign: "center", fontSize: "13px", color: "oklch(45% 0 0)", fontFamily: "var(--font-sans)" }}>{debouncedTerm ? "No meetings found" : "No recent meetings"}</div>}
            {!isLoading && meetings?.map((m) => (
              <button
                key={m.id} type="button" disabled={!!loadingId}
                onClick={() => setLoadingId(m.id)}
                style={{ width: "100%", display: "flex", flexDirection: "column", alignItems: "flex-start", gap: "2px", padding: "10px 14px", background: loadingId === m.id ? "oklch(68% 0.12 75 / 0.1)" : "transparent", border: "none", borderBottom: "1px solid oklch(16% 0 0)", cursor: loadingId ? "not-allowed" : "pointer", textAlign: "left", transition: "background 0.15s" }}
              ...
              >
                <span style={{ fontSize: "13px", color: "oklch(88% 0 0)", fontFamily: "var(--font-sans)", fontWeight: 500 }}>{loadingId === m.id ? "Importing..." : m.title}</span>
                {m.date && <span style={{ fontSize: "11px", color: "oklch(45% 0 0)", fontFamily: "var(--font-sans)" }}>{new Date(m.date).toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric" })}</span>}
              </button>
            ))}
          </div>
          <div style={{ padding: "8px 12px", borderTop: "1px solid oklch(18% 0 0)" }}>
            <p style={{ fontSize: "11px", color: "oklch(38% 0 0)", fontFamily: "var(--font-sans)", margin: 0 }}>Showing your recent Fireflies meetings</p>
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <>
      ...
      <button
        ...
        style={{
          display: "flex", alignItems: "center", gap: "6px", padding: "7px 12px",
          borderRadius: "8px", background: "oklch(14% 0 0)",
          border: "1px solid oklch(24% 0 0)",
          color: isProcessing ? "oklch(55% 0 0)" : "oklch(65% 0 0)",
...
function PasteImportModal({ onImport, onClose }: { onImport: (text: string) => void; onClose: () => void }) {
  ...
  return (
    <div
      style={{ position: "fixed", inset: 0, background: "oklch(0% 0 0 / 0.7)", zIndex: 200, display: "flex", alignItems: "flex-end", justifyContent: "center", padding: "0 0 20px" }}
      onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}
    >
      <div style={{ width: "100%", maxWidth: "640px", background: "oklch(12% 0 0)", border: "1px solid oklch(22% 0 0)", borderRadius: "16px 16px 12px 12px", padding: "20px", boxShadow: "0 -8px 40px oklch(0% 0 0 / 0.5)" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "14px" }}>
          <span style={{ fontFamily: "var(--font-serif)", fontSize: "16px", fontWeight: 600, color: "oklch(90% 0 0)" }}>Paste from Note App</span>
          <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", color: "oklch(45% 0 0)", padding: "4px" }}><X size={18} /></button>
        </div>
        <p style={{ fontSize: "12px", color: "oklch(45% 0 0)", fontFamily: "var(--font-sans)", marginBottom: "12px" }}>
        ...
        <textarea
          ...
          rows={8}
          style={{
            width: "100%", background: "oklch(10% 0 0)", border: "1px solid oklch(26% 0 0)",
        ...
        <div style={{ display: "flex", gap: "10px", marginTop: "12px" }}>
          <button
            onClick={onClose}
            style={{ flex: 1, padding: "11px", borderRadius: "10px", background: "oklch(16% 0 0)", border: "1px solid oklch(24% 0 0)", color: "oklch(65% 0 0)", fontSize: "14px", fontFamily: "var(--font-sans)", cursor: "pointer" }}
          >
            Cancel
          </button>
          <button
            ...
            disabled={!text.trim()}
            style={{ flex: 2, padding: "11px", borderRadius: "10px", background: text.trim() ? "oklch(68% 0.12 75)" : "oklch(30% 0 0)", border: "none", color: text.trim() ? "#0e0e0e" : "oklch(45% 0 0)", fontSize: "14px", fontWeight: 700, fontFamily: "var(--font-sans)", cursor: text.trim() ? "pointer" : "not-allowed" }}
          >
            Import Notes
          </button>
        </div>
    ...
    <div>
        ...
        {PRESET_TAGS.map((tag) => {
          ...
          return (
            <button key={tag} type="button" onClick={() => toggle(tag)}
              style={{ display: "flex", alignItems: "center", gap: "4px", padding: "5px 10px", borderRadius: "20px", background: active ? "oklch(68% 0.12 75 / 0.18)" : "oklch(14% 0 0)", border: `1px solid ${active ? "oklch(68% 0.12 75 / 0.6)" : "oklch(22% 0 0)"}`, color: active ? "oklch(68% 0.12 75)" : "oklch(55% 0 0)", fontSize: "12px", fontWeight: active ? 600 : 400, fontFamily: "var(--font-sans)", cursor: "pointer" }}
            >
              {active && <X size={10} />}
              {tag}
            </button>
      ...
      <div style={{ display: "flex", gap: "6px" }}>
        <input
          ...
          style={{ flex: 1, background: "oklch(13% 0 0)", border: "1px solid oklch(22% 0 0)", borderRadius: "8px", padding: "7px 10px", fontSize: "12px", color: "oklch(88% 0 0)", fontFamily: "var(--font-sans)", outline: "none" }}
        ...
        <button type="button" onClick={addCustom} disabled={!customInput.trim()}
          style={{ padding: "7px 12px", borderRadius: "8px", background: "oklch(14% 0 0)", border: "1px solid oklch(22% 0 0)", color: "oklch(60% 0 0)", fontSize: "12px", fontFamily: "var(--font-sans)", cursor: customInput.trim() ? "pointer" : "not-allowed", opacity: customInput.trim() ? 1 : 0.5 }}
        >
          Add
        </button>
      </div>
      {selected.filter((t) => !PRESET_TAGS.includes(t)).length > 0 && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: "6px", marginTop: "6px" }}>
          {selected.filter((t) => !PRESET_TAGS.includes(t)).map((tag) => (
            <span key={tag} style={{ display: "flex", alignItems: "center", gap: "4px", padding: "4px 8px", borderRadius: "20px", background: "oklch(68% 0.12 75 / 0.12)", border: "1px solid oklch(68% 0.12 75 / 0.4)", color: "oklch(68% 0.12 75)", fontSize: "12px", fontFamily: "var(--font-sans)" }}>
              {tag}
    ...
    <AppShell>
      <div className="container py-6">
        ...
        <div className="flex items-center gap-3 mb-6">
          <button
            onClick={() => navigate("/")}
            style={{ background: "oklch(14% 0 0)", border: "1px solid oklch(22% 0 0)", borderRadius: "10px", width: "38px", height: "38px", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0 }}
          >
            <ArrowLeft size={18} style={{ color: "oklch(65% 0 0)" }} />
          </button>
          <div>
            <h1 style={{ fontFamily: "var(--font-serif)", fontSize: "20px", fontWeight: 600, color: "oklch(92% 0 0)", margin: 0 }}>New Session</h1>
            <p style={{ fontSize: "12px", color: "oklch(45% 0 0)", margin: 0, fontFamily: "var(--font-sans)" }}>Capture your meeting intelligence</p>
          </div>
        </div>
        ...
        <div className="mb-5">
          ...
          <input
            ...
            style={{ width: "100%", background: "oklch(13% 0 0)", border: "1px solid oklch(22% 0 0)", borderRadius: "10px", padding: "13px 14px", fontSize: "15px", color: "oklch(90% 0 0)", fontFamily: "var(--font-serif)", outline: "none", transition: "border-color 0.2s" }}
        ...
        <div className="mb-5">
          ...
          <textarea
            ...
            rows={7}
            style={{ width: "100%", background: "oklch(13% 0 0)", border: "1px solid oklch(22% 0 0)", borderRadius: "10px", padding: "13px 14px", fontSize: "14px", color: "oklch(88% 0 0)", fontFamily: "var(--font-sans)", outline: "none", resize: "vertical", lineHeight: 1.6, transition: "border-color 0.2s" }}
            onFocus={(e) => (e.target.style.borderColor = "oklch(68% 0.12 75 / 0.6)")}
            onBlur={(e) => (e.target.style.borderColor = "oklch(22% 0 0)")}
          />
          {transcript && (
        ...
        <div className="mb-6">
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "8px", flexWrap: "wrap", gap: "6px" }}>
            <label style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "11px", fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", color: "oklch(68% 0.12 75)", fontFamily: "var(--font-sans)" }}>
              <Lock size={12} />
              Private Notes
            </label>
            {/* Import buttons */}
            <div style={{ display: "flex", gap: "6px" }}>
              ...
              <button
                ...
                style={{ display: "flex", alignItems: "center", gap: "6px", padding: "7px 12px", borderRadius: "8px", background: "oklch(14% 0 0)", border: "1px solid oklch(24% 0 0)", color: "oklch(65% 0 0)", fontSize: "12px", fontWeight: 500, fontFamily: "var(--font-sans)", cursor: "pointer", whiteSpace: "nowrap" }}
              >
          ...
          <textarea
            ...
            rows={6}
            style={{ width: "100%", background: "oklch(11.5% 0.005 75)", border: "1px solid oklch(68% 0.12 75 / 0.2)", borderRadius: "10px", padding: "13px 14px", fontSize: "14px", color: "oklch(88% 0 0)", fontFamily: "var(--font-sans)", outline: "none", resize: "vertical", lineHeight: 1.6, transition: "border-color 0.2s" }}
        ...
        <div className="flex gap-3">
          <button
            ...
            style={{ flex: 1, padding: "13px", borderRadius: "10px", background: "oklch(14% 0 0)", border: "1px solid oklch(24% 0 0)", color: "oklch(70% 0 0)", fontSize: "14px", fontWeight: 500, fontFamily: "var(--font-sans)", cursor: "pointer", transition: "all 0.2s" }}
          >
            Save Draft
          </button>
          <button
            ...
            disabled={isAnalyzing || createSession.isPending}
            style={{ flex: 2, padding: "13px", borderRadius: "10px", background: isAnalyzing ? "oklch(50% 0.08 75)" : "oklch(68% 0.12 75)", border: "none", color: "#0e0e0e", fontSize: "15px", fontWeight: 700, fontFamily: "var(--font-sans)", cursor: isAnalyzing ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: "8px", boxShadow: isAnalyzing ? "none" : "0 4px 16px oklch(68% 0.12 75 / 0.25)", transition: "all 0.2s" }}
          >
            {isAnalyzing ? (
